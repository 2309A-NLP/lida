...
class RAGEngine:
    ...
    def __init__(self, config: dict):
        ...
        self.embedder = Embedder(
            backend=emb_cfg.get("backend", "openai"),
        ...
        self.similarity_threshold = config["retrieval"].get("similarity_threshold", 0.30)
        ...
        self._persistent_cache = PersistentCache(
            "/home/lida/query_cache.db", max_entries=200
    ...
    def query(self, question: str) -> dict:
        ...
        gen_start = time.time()
        if not hits:
            answer = "未在文档中找到与问题相关的信息。"
        else:
            answer = self.llm.generate_with_rag(question, hits)
        gen_time = time.time() - gen_start

        llm_start = time.time()
        llm_answer = self.llm.generate_directly(question)
    ...
    def query_stream(self, question: str):
        ...
        # 阶段1: 检索
        yield {"type": "phase", "phase": "retrieval", "message": "正在检索相关文档..."}
        hits, pages = self._retrieve(question)
        retrieval_time = time.time() - start_time

        if not hits:
            yield {"type": "phase", "phase": "done", "message": "未找到相关文档"}
            answer = "未在文档中找到与问题相关的信息。"
            result = {
                "question": question, "answer": answer,
                "llm_only_answer": "", "retrieved_chunks": [],
                "pages": [], "retrieval_time": round(retrieval_time, 3),
                "generation_time": 0, "llm_only_time": 0,
                "total_time": round(time.time() - start_time, 3), "cached": False,
            }
        ...
        gen_start = time.time()
        collected_answer = []
        for token in self.llm.generate_with_rag_stream(question, hits):
    ...
    def query_with_understanding(self, question: str) -> dict:
        """带Query理解的RAG问答"""
        start_time = time.time()
        cached = self._check_caches(question)
        if cached:
        ...
        if not hits and not is_short_query:
            if not raw_hits and not is_short_query:
        ...
        if qu_cfg.get("decomposition", True) and analysis.get("is_complex"):
            try:
                return self._handle_complex(question, analysis, start_time)
            except Exception:
                pass
    ...
    def _handle_complex(self, question: str, analysis: dict, start_time: float) -> dict:
        """处理复杂问题：分解为子问题"""
