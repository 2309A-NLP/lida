#!/usr/bin/env python3
"""Lightweight embedding server compatible with OpenAI API format.
Serves bge-small-zh-v1.5 on port 11434."""
...
print("Loading bge-small-zh-v1.5 model...", flush=True)
...
class EmbeddingHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == '/v1/models':
            ...
            self.send_header('Content-Type', 'application/json')
            self.end_headers()
            resp = {"models": [{"name": "bge-small-zh-v1.5"}]}
            self.wfile.write(json.dumps(resp).encode())
        else:
