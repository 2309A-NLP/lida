...
def main():
    ...
    for q in QUESTIONS:
        ...
        # 构建极简上下文
        context = "\n".join([c['content'][:200] for c in chunks[:1]])
        ...
        print(f"Q{q['id']}: {status} {reason} | {total:.1f}s (检索:{ret_time:.1f}s LLM:{llm_time:.1f}s)")
    ...
    print(f"所有时间: {[f'{t:.1f}' for t in times]}")
