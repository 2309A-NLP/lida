...
try:
    ...
    print(f"GPT4Free模型加载失败，可以忽略: {e}")
from .QAnything import QAnything
from .LegalRAG import LegalRAG
...
class LLM:
    ...
    def init_model(self, model_name, model_path='', api_key=None, proxy_url=None, prefix_prompt='''请用少于25个字回答以下问题\n\n'''):
        if model_name not in ['Linly', 'Qwen', 'Qwen2', 'Gemini', 'ChatGLM', 'ChatGPT', 'Llama2Chinese', 'GPT4Free', 'QAnything', 'LegalRAG', '直接回复 Direct Reply']:
            raise ValueError("model_name must be one of ['Linly', 'Qwen', 'Qwen2', 'Gemini', 'ChatGLM', 'ChatGPT', 'Llama2Chinese', 'GPT4Free', 'QAnything', 'LegalRAG', '直接回复 Direct Reply']")
        if model_name == 'Linly':
            ...
            llm = LegalRAG(model_path=model_path, api_key=api_key, proxy_url=proxy_url)
