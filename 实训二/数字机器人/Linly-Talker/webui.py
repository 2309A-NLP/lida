...
def get_title(title = '法小助 - 法律数字人咨询系统 (Legal-AI)'):
    description = f"""
    <p style="text-align: center; font-weight: bold;">
        <span style="font-size: 28px;">⚖️ {title}</span>
        <br>
        ...
        <br> 
        <span>⚖️ 法小助是一款基于RAG的法律数字人咨询系统。融合LLM + RAG法律知识检索，提供准确的法律条文引用和专业的法律咨询服务。支持语音交互、数字人形象。</span>
    </p>
    """
    return description

# Default system and prompt settings
DEFAULT_SYSTEM = '你是「法小助」——专业、严谨的中国法律咨询助手。根据中国现行法律法规回答法律问题，引用具体法律条款。回答结构：法条引用 → 解读 → 建议。你提供的只是法律信息参考，不构成正式法律意见。'
...
GPT_SoVITS_ckpt = "/root/autodl-tmp/Linly-Talker/GPT_SoVITS/pretrained_models"
...
def webui_setting(talk=False):
    ...
    llm_method = gr.Dropdown(choices=['Qwen', 'Qwen2', 'Linly', 'Gemini', 'ChatGLM', 'ChatGPT', 'GPT4Free', 'QAnything', 'LegalRAG', '直接回复 Direct Reply'], value='LegalRAG', label='LLM 模型选择')
...
def exmaple_setting(asr, text, character, talk, tts, voice, llm):
    # 默认text的Example
    examples = [
        ['Whisper-base', '我在工作中受伤了，算工伤吗？', '女性角色', 'SadTalker', 'Edge-TTS', 'zh-CN-XiaoxiaoNeural', 'LegalRAG'],
        ['Whisper-tiny', '离婚财产怎么分割？', '女性角色', 'SadTalker', 'PaddleTTS', 'None', 'LegalRAG'],
...
def llm_model_change(model_name, progress=gr.Progress(track_tqdm=True)):
    ...
    try:
        ...
        elif model_name == 'LegalRAG':
            llm = llm_class.init_model('LegalRAG')
            gr.Info("法律RAG检索增强模型加载成功")
        else:
...
if __name__ == "__main__":
    ...
    demo.launch(
        server_name="0.0.0.0",  # 本地localhost:127.0.0.1 或 "0.0.0.0" 进行全局端口转发
        server_port=port,
